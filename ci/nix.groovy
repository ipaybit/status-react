...
def shell(Map args = [:], String cmd) {
  ...
  args.keep = args.keep + defaults.keep

  def isPure = args.pure && env.TARGET_OS != 'windows' && env.TARGET_OS != 'ios'
  def keepFlags = args.keep.collect { var -> "--keep ${var} " }
  def argsFlags = args.args.collect { key,val -> "--argstr ${key} \'${val}\'" }
  sh """
    set +x
